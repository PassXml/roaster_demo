...
fun main() {
    val scriptFile = "/Volumes/Project/Project/self/java/agent_test/src/test/kotlin/test.kt"

    val parser = KotlinScriptParser()

    val analyzeContext = parser.parse(scriptFile)

    for (function in analyzeContext.functions) {
        println(function.key.name)
        for (parameter in function.value.valueParameters) {
            println(parameter.name)
            println(parameter.index)
            for (argument in parameter.type.arguments) {
